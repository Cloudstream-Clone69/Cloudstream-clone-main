...
function _decodeFully(str) {
  ...
  return decoded;
}function _patchAndCleanUrl(urlStr, cookieStr) {
  if (!urlStr) return urlStr;
  let targetUrl = urlStr;
  
  if (targetUrl.includes('in=unknown')) {
    try {
      let token = '';
      if (cookieStr) {
        const match = cookieStr.match(/t_hash_(p|t)=([^;]+)/);
        if (match) {
          token = match[2];
      ...
      if (token) {
        const decodedToken = _decodeFully(token);
        targetUrl = targetUrl.replace(/in=unknown(::ni)?/g, `in=${encodeURIComponent(decodedToken)}`);
      } else {
  ...
  if (targetUrl.includes('in=')) {
    try {
      const urlObj = new URL(targetUrl);
      const inVal = urlObj.searchParams.get('in');
      if (inVal && inVal !== 'unknown' && inVal !== 'unknown::ni') {
        const decodedIn = _decodeFully(inVal);
        const parts = decodedIn.split(':').map(p => p.trim()).filter(Boolean);
        const cleaned = parts.join('::');
        urlObj.searchParams.set('in', cleaned);
        targetUrl = urlObj.toString();
      }
    } catch (_) {}
  }
...
// Returns true for net52/freecdn CDN URLs that require the in= auth token
function _isNet52Cdn(url) {
  try {
    const h = new URL(url).hostname.toLowerCase();
    return h.includes('freecdn') || h.includes('net52') || h.includes('nm-cdn') ||
           h.includes('chittiman') || h.includes('imgcdn.kim') || h.includes('subscdn');
  } catch (_) { return false; }
}

async function fetchWithWarpFallback(url, options) {
  try {
    // Netmirror CDN strictly validates the client's connecting IP against the IP hash
    // embedded in the session token. Routing requests through WARP SOCKS5 causes an IP mismatch
    // (warning video plays instead of content). Force direct connection for Netmirror hosts.
    let isNetmirror = false;
    try {
      const host = new URL(url).hostname.toLowerCase();
      isNetmirror = host.includes('net77.cc') || host.includes('net22.cc') || 
                    host.includes('imgcdn.kim') || host.includes('freecdn') || 
                    host.includes('subscdn.top') || host.includes('nm-cdn') || 
                    url.includes('/mobile/') || url.includes('/newtv/') ||
                    url.includes('/stream2/') || url.includes('/playlist/') ||
                    host.includes('slash') || host.includes('gemma') ||
                    host.includes('chittiman.space');
    } catch (_) {}
...
class PngVideoFilter extends Transform {
  constructor() {
    super();
    this.checked = false;
  }
  _transform(chunk, encoding, callback) {
    if (!this.checked) {
      ...
      if (chunk.length >= 8 &&
          ...
          chunk[6] === 0x1a &&
          chunk[7] === 0x0a) {
        console.log('[Segment] PNG disguise detected, stripping 70 bytes');
        if (chunk.length > 70) {
          this.push(chunk.slice(70));
        }
        return callback();
      }
    }
...
function _decryptCencSegment(inputBuf, keyHex) {
  ...
  for (let i = 0; i < topBoxes.length; i++) {
    ...
    // Detect IV size (8 or 16 bytes)
    let ivSize = 8;
    if (sCount > 0) {
      const payloadLen = senc.e - (sData + 8);
      if (!useSubs) {
        if (payloadLen >= 16 * sCount) {
          ivSize = 16;
        }
      } else {
        if (sData + 8 + 8 + 2 <= senc.e) {
          const testNSubs = _ru16(buf, sData + 8 + 8);
          const expectedLen = 8 + 2 + testNSubs * 6;
          if (expectedLen > payloadLen || (sCount === 1 && expectedLen !== payloadLen)) {
            ivSize = 16;
          }
        }
      }
    ...
    // Parse samples from SENC
    const samples = [];
    for (let s = 0; s < sCount; s++) {
      if (sp + ivSize > senc.e) break;
      const iv = Buffer.alloc(16, 0);
      buf.copy(iv, 0, sp, sp + ivSize);
      sp += ivSize;
      let subs = null;
      if (useSubs) {
        if (sp + 2 > senc.e) break;
...
// anidb.app CDN and sports CDNs require Android Dalvik UA to bypass Akamai/CloudFront geoblocks.
// All other CDNs use a standard browser UA.
function getUA(url) {
    try {
        const host = new URL(url).hostname;
        if (host.includes('anidb.app') || host.includes('aiv-cdn.net') || host.includes('pv-cdn.net')) {
...
function resolveUA(url, queryUa) {
  const ua = getUA(url);
  if (ua.includes('Dalvik')) return ua;
  return queryUa || ua;
}
...
function _injectNetmirrorHeaders(url, headers, forcedOtt) {
  try {
    ...
    const isNetmirrorHost = host.includes('net77.cc') || host.includes('net22.cc') || 
                            host.includes('imgcdn.kim') || host.includes('freecdn') || 
                            host.includes('subscdn.top') || host.includes('nm-cdn') || 
                            url.includes('/mobile/') || url.includes('/newtv/') ||
                            url.includes('/stream2/') || url.includes('/playlist/') ||
                            host.includes('slash') || host.includes('gemma') ||
                            host.includes('chittiman.space') || forcedOtt;
    if (isNetmirrorHost) {
      ...
      headers['ott'] = ott;
      let netmirrorUserAgent = 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/124.0.0.0 Safari/537.36';
      try {
        const settingsPath = path.join(process.cwd(), 'app-settings.json');
        if (fs.existsSync(settingsPath)) {
          const settings = JSON.parse(fs.readFileSync(settingsPath, 'utf8'));
          if (settings.netmirrorUserAgent) {
            netmirrorUserAgent = settings.netmirrorUserAgent;
          }
        }
      } catch (_) {}
      headers['User-Agent'] = netmirrorUserAgent;
      headers['X-Requested-With'] = 'app.netmirror.netmirrornew';
      if (!headers['Referer'] || headers['Referer'].includes('net77.cc') || headers['Referer'].includes('mobile/home')) {
        headers['Referer'] = `https://net52.cc/${ott}/player.php`;
      }
      if (!headers['Origin'] || headers['Origin'].includes('net77.cc')) {
...
/**
 * GET /proxy/hls-quality
 * On-demand quality resolver: calls getPlaylist() FRESH every time so the
 * in= token is never stale, regardless of how long the user waited before
 * switching quality. Redirects (307) to /proxy/hls with the fresh URL.
 *
 * Params: id, ott, q (quality string e.g. '1080p'), cookie, ref, ua
 */
router.get("/hls-quality", async (req, res) => {
    const contentId = req.query.id;
    const ott       = req.query.ott || 'nf';
    const quality   = req.query.q || '';
    const cookie    = _decodeFully(req.query.cookie || '');
    const referer   = _decodeFully(req.query.ref || 'https://net52.cc/mobile/home?app=1');
    const ua        = _decodeFully(req.query.ua || '');

    if (!contentId) return res.status(400).send('Missing id param');

    try {
        console.log(`[hls-quality] Fresh token fetch for id=${contentId} ott=${ott} q=${quality}`);
        const playlist = await getPlaylist(contentId, contentId, ott);

        // Find the matching quality source (by q= param or fall back to auto)
        const qualitySource = playlist.sources.find(s => s.url.includes(`q=${quality}`))
                           || playlist.sources.find(s => !s.url.includes('q='))
                           || playlist.sources[0];

        if (!qualitySource) return res.status(404).send('Quality not found in playlist');

        // Build fresh cookie string from the new playlist.token
        const freshCookie = `t_hash_t=${encodeURIComponent(playlist.token)}; ott=${ott}; hd=on;`;

        // Redirect to the standard HLS proxy with the FRESH quality URL + fresh cookie.
        // 307 preserves the GET method. ExoPlayer/ExoPlayer2 follow 307 for HLS manifests.
        const freshProxyUrl = `/proxy/hls?url=${encodeURIComponent(qualitySource.url)}`
            + `&ref=${encodeURIComponent(referer)}`
            + `&cookie=${encodeURIComponent(freshCookie)}`
            + `&ott=${encodeURIComponent(ott)}`
            + (ua ? `&ua=${encodeURIComponent(ua)}` : '');

        console.log(`[hls-quality] Redirecting to fresh ${quality} manifest for ${contentId}`);
        return res.redirect(307, freshProxyUrl);
    } catch (err) {
        console.error('[hls-quality] Error:', err.message);
        return res.status(500).send('Failed to get fresh quality stream: ' + err.message);
    }
});
...
router.get("/hls", async (req,res)=>{
    ...
    try{
        ...
        let target = _safeUrl(req.query.url || "");
        // Guard: net52 playlist.php sometimes returns sources with a bare "files" hostname
        // (e.g. https://files/ID/720p/720p.m3u8) which causes ENOTFOUND. Remap to net52.cc.
        try {
            const _tParsed = new URL(target);
            if (_tParsed.hostname === 'files' || (_tParsed.hostname && !_tParsed.hostname.includes('.'))) {
                console.warn(`[HLS] Fixing malformed CDN hostname "${_tParsed.hostname}" → net52.cc`);
                target = 'https://net52.cc' + _tParsed.pathname + _tParsed.search;
            }
        } catch (_) {}
        console.log(`[HLS] Requesting: ${target.split('/').pop().split('?')[0]} (${target.includes('?in=') ? 'authenticated' : 'NO_AUTH'})`);
        const referer = _decodeFully(req.query.ref || "");
        const cookie  = _decodeFully(req.query.cookie || "");
        let streamToken = '';
        try {
        ...
        const queryUa = _decodeFully(req.query.ua || "");
        const queryOrigin = req.query.origin || "";
        let customUa = resolveUA(target, queryUa);
        ...
        const actualTarget = _patchAndCleanUrl(target, cookie);
        console.log(`[HLS] Requesting target: ${actualTarget.substring(0, 120)}...`);
        const response = await fetchWithWarpFallback(actualTarget,{ headers });
        const text = typeof response.data === 'string' ? response.data : (response.data ? response.data.toString('utf8') : '');
        ...
        if (text.includes('#EXT-X-STREAM-INF') || text.includes('#EXT-X-I-FRAME-STREAM-INF')) {
            ...
            const rewritten = lines.map(line => {
                ...
                if (!trimmed || trimmed.startsWith('#')) {
                    ...
                    return line.replace(/URI="([^"]+)"/g, (_, uri) => {
                        ...
                        if (streamToken) {
                            if (absolute.includes('in=unknown')) {
                                absolute = absolute.replace(/in=unknown(::ni)?/g, `in=${encodeURIComponent(streamToken)}`);
                            } else if (!absolute.includes('in=') && _isNet52Cdn(absolute)) {
                                absolute += (absolute.includes('?') ? '&' : '?') + 'in=' + encodeURIComponent(streamToken);
                            }
                        } else {
                            absolute = _patchAndCleanUrl(absolute, cookie);
                        }
                        if (queryParams && !absolute.includes('?')) {
                            absolute += queryParams;
                        }
                        return `URI="${globalThis.APP_URL}/proxy/hls?url=${encodeURIComponent(absolute)}&ref=${encodeURIComponent(referer)}${cookieSuffix}&ott=${ott}${uaSuffix}${originSuffix}${sportsSuffix}"`;
                    });
                }
                // Variant URL line — rewrite through proxy
                let absolute = abs(target, trimmed);
                if (streamToken) {
                    if (absolute.includes('in=unknown')) {
                        absolute = absolute.replace(/in=unknown(::ni)?/g, `in=${encodeURIComponent(streamToken)}`);
                    } else if (!absolute.includes('in=') && _isNet52Cdn(absolute)) {
                        // Quality variant (720p, 480p etc.) from net52/freecdn CDN has no in= token
                        // — inject from the master manifest's streamToken to prevent warning video
                        absolute += (absolute.includes('?') ? '&' : '?') + 'in=' + encodeURIComponent(streamToken);
                    }
                } else {
                    absolute = _patchAndCleanUrl(absolute, cookie);
                }
                return `${globalThis.APP_URL}/proxy/hls?url=${encodeURIComponent(absolute)}&ref=${encodeURIComponent(referer)}${cookieSuffix}&ott=${ott}${uaSuffix}${originSuffix}${sportsSuffix}`;
            }).join('\n');
            ...
            // ── QUALITY PINNING ──────────────────────────────────────────────────
            // Net52 returns a FULL master manifest with ALL variants (1080p, 720p, 480p)
            // even when ?q=480p is requested. ExoPlayer then auto-picks the highest
            // (always 1080p). Fix: strip all OTHER quality variants so ExoPlayer is
            // forced to play exactly the quality the user selected.
            try {
                const targetUrl = new URL(target);
                const requestedQuality = targetUrl.searchParams.get('q') || '';

                if (requestedQuality) {
                    const QUALITY_HEIGHT = {
                        '4k': 2160, '2160p': 2160,
                        '1080p': 1080, 'full hd': 1080, 'fhd': 1080,
                        '720p': 720,  'mid hd': 720,  'mhd': 720,
                        '480p': 480,  'low hd': 480,  'lhd': 480, 'sd': 480,
                        '360p': 360,  '240p': 240
                    };
                    const requestedHeight = QUALITY_HEIGHT[requestedQuality.toLowerCase()] || 0;

                    if (requestedHeight > 0) {
                        // Collect all STREAM-INF blocks
                        const playlistLines = finalPlaylist.split('\n');
                        const blocks = [];
                        for (let i = 0; i < playlistLines.length; i++) {
                            if (playlistLines[i].includes('#EXT-X-STREAM-INF')) {
                                const resMatch = playlistLines[i].match(/RESOLUTION=(\d+)x(\d+)/i);
                                const height = resMatch ? parseInt(resMatch[2], 10) : 0;
                                blocks.push({ infLine: playlistLines[i], urlLine: playlistLines[i + 1] || '', height });
                            }
                        }

                        if (blocks.length > 1) {
                            // Find closest match (prefer exact, then nearest <=, then nearest above)
                            let best = null;
                            for (const b of blocks) {
                                if (!best) { best = b; continue; }
                                const diff    = Math.abs(b.height - requestedHeight);
                                const bestDiff = Math.abs(best.height - requestedHeight);
                                if (diff < bestDiff) best = b;
                                else if (diff === bestDiff && b.height <= requestedHeight && best.height > requestedHeight) best = b;
                            }

                            if (best) {
                                // Rebuild playlist keeping only the chosen variant
                                const newLines = [];
                                ...
                                for (const line of playlistLines) {
                                    ...
                                    if (line.includes('#EXT-X-STREAM-INF') || line.includes('#EXT-X-I-FRAME-STREAM-INF')) {
                                        if (line === best.infLine) {
                                            newLines.push(line);
                                        } else {
                                            skipNextUrl = true; // skip inf + its url
                                        }
                                    } else {
                                        newLines.push(line);
                                    }
                                }
                                finalPlaylist = newLines.join('\n');
                                console.log(`[HLS] Quality-pinned to ${requestedQuality} → ${best.height}p (from ${blocks.length} variants)`);
                            }
                        }
                    }
                }
            } catch (qErr) {
                console.warn('[HLS] Quality pinning error (non-fatal):', qErr.message);
            }
        ...
        const rewritten = text
            ...
            .map(line=>{
                ...
                if(trimmed.startsWith("#")){
                    ...
                    return line.replace(
                        ...
                        (_,uri)=>{
                            ...
                            const ext = (segUrl.split('?')[0].split('.').pop() || '').toLowerCase();
                            const suffix = (ext === 'js' || ext === 'ts') ? '/stream.ts' : (['m4s', 'mp4', 'm4a', 'm4v', 'woff', 'woff2'].includes(ext) ? '/stream.mp4' : '');

                            return `URI="${globalThis.APP_URL}/proxy/segment${suffix}?url=${encodeURIComponent(segUrl)}&ref=${encodeURIComponent(referer)}${cookieSuffix}&ott=${ott}${uaSuffix}${originSuffix}${sportsSuffix}"`;
                        }
                    );
                }
                ...
                const ext = (segUrl.split('?')[0].split('.').pop() || '').toLowerCase();
                const suffix = (ext === 'js' || ext === 'ts') ? '/stream.ts' : (['m4s', 'mp4', 'm4a', 'm4v', 'woff', 'woff2'].includes(ext) ? '/stream.mp4' : '');
                return `http://127.0.0.1:3000/proxy/segment${suffix}?url=${encodeURIComponent(segUrl)}&ref=${encodeURIComponent(referer)}${cookieSuffix}&ott=${ott}${uaSuffix}${originSuffix}${sportsSuffix}`;
        ...
        if (req.query.sports === '1') {
            // For live sports, force-remove #EXT-X-ENDLIST and #EXT-X-PLAYLIST-TYPE
            // before applying the sliding window. Some CDNs emit these on short-window
            // live playlists; leaving them causes MPV to treat the stream as finished VOD
            // (plays 2-3 segs then stops). Stripping them makes MPV keep polling.
            let sportsPlaylist = rewritten
                .replace(/^#EXT-X-ENDLIST\s*$/gm, '')
                .replace(/^#EXT-X-PLAYLIST-TYPE:.*$\s*/gm, '')
                .replace(/\n{3,}/g, '\n\n'); // collapse blank lines
            finalPlaylist = _enforceHlsSlidingWindow(sportsPlaylist, 15);
        }
...
router.get(/^\/segment/, async (req,res)=>{
    ...
    try{
        ...
        let referer = _decodeFully(req.query.ref || '');
        let cookie  = _decodeFully(req.query.cookie || '');
        const queryOtt = req.query.ott || '';
        let sessionUa = '';
        let sessionOrigin = '';
        if (sessionId) {
        ...
        const queryUa = _decodeFully(req.query.ua || '') || sessionUa || "";
        ...
        const headers = {
            ...
            "User-Agent": resolveUA(target, queryUa),
        ...
        let actualTarget = _patchAndCleanUrl(target, cookie);
        console.log(`[Segment] Requesting target: ${actualTarget.substring(0, 120)}...`);
        try {
            const targetHost = new URL(actualTarget).hostname;
        ...
        if (['ts', 'm4s', 'mp4', 'm4a', 'm4v', 'woff', 'woff2', 'jpg', 'jpeg', 'js', 'png', 'css'].includes(ext)) {
          const mime = (ext === 'ts' || ext === 'jpg' || ext === 'jpeg' || ext === 'js' || ext === 'png' || ext === 'css') ? 'video/mp2t' : (ext === 'm4s' || ext === 'mp4' ? 'video/mp4' : ct);
          ...
          const filter = new PngVideoFilter();
          await new Promise((resolve, reject) => {
  ...
  if (periodMatch) {
    ...
    if (pStFullM) {
      ...
      while ((ps = pSRx.exec(pContent)) !== null) {
        ...
        const tM  = sEl.match(/\bt="(\d+)"/);
        if (!dM) continue;
        const d = parseInt(dM[1]);
        const r = rM ? parseInt(rM[1]) : 0;
        if (tM) pCurrentT = parseInt(tM[1]); // explicit jump in timestamp
        const count = r < 0 ? 1 : r + 1;
        for (let i = 0; i < count; i++) { pSegments.push({ d, t: pCurrentT }); pCurrentT += d; }
      }
      ...
      if (pSegments.length > 0)
        console.log(`[DASH→HLS] Period-level SegmentTemplate found: ${pSegments.length} segments (${periodST.mediaTmpl.includes('$Time$') ? '$Time$' : '$Number$'} mode)`);
    }
  }
  ...
  while ((m = rx.exec(mpd)) !== null) {
    ...
    while ((rm = reprRx.exec(inner)) !== null) {
      ...
      while ((sm = sRx.exec(searchTarget)) !== null) {
        ...
        const d = parseInt(dM[1]);
        const r = rM ? parseInt(rM[1]) : 0;
        if (tM) currentT = parseInt(tM[1]); // explicit timestamp jump
        const count = r < 0 ? 1 : r + 1;
        for (let i = 0; i < count; i++) { segments.push({ d, t: currentT }); currentT += d; }
      }
      ...
      if (finalSegments.length === 0) {
        ...
        if (stDuration > 0) {
          // For live streams: generate a sliding window of segments using wall-clock time
          // For VOD streams: use mediaPresentationDuration
          const durStr = mpd.match(/mediaPresentationDuration="PT([^"]+)"/)?.[1] || '';
          const isDynamic = /type\s*=\s*"dynamic"/i.test(mpd);
          ...
          if (durStr && !isDynamic) {
            ...
            if (totalSec > 0) {
              ...
              let tAcc = 0;
              finalSegments = Array.from({ length: numSegs }, () => { const seg = { d: stDuration, t: tAcc }; tAcc += stDuration; return seg; });
              console.log(`[DASH→HLS] Fixed-duration segments (VOD): ${numSegs} × ${segDurSec}s = ${(numSegs * segDurSec).toFixed(1)}s`);
            }
          } else {
            // Live mode: generate segments based on current time + startNumber
            // availabilityStartTime + presentationTimeOffset gives us current segment number
            const LIVE_WIN = 30; // Keep 30 segments in HLS window
            const availabilityStartMatch = mpd.match(/availabilityStartTime="([^"]+)"/);
            const presentationTimeOffsetMatch = stAttrs.match(/presentationTimeOffset="(\d+)"/);
            const pto = parseInt(presentationTimeOffsetMatch?.[1] || '0');

            let currentSegNum = startNum;
            if (availabilityStartMatch) {
              const astMs = Date.parse(availabilityStartMatch[1]);
              if (!isNaN(astMs)) {
                const nowMs = Date.now();
                const elapsedTicks = Math.floor((nowMs - astMs) / 1000 * timescale);
                const elapsedWithPTO = elapsedTicks + pto;
                const segsElapsed = Math.floor(elapsedWithPTO / stDuration);
                currentSegNum = startNum + segsElapsed;
              }
            }

            // Generate LIVE_WIN segment stubs around the live point
            let tAcc = Math.max(0, currentSegNum - startNum - LIVE_WIN) * stDuration;
            const fromSeg = Math.max(startNum, currentSegNum - LIVE_WIN);
            const count = LIVE_WIN + 3; // slight buffer ahead
            finalSegments = Array.from({ length: count }, () => {
              const seg = { d: stDuration, t: tAcc };
              tAcc += stDuration;
              return seg;
            });
            finalSegments._liveStartNum = fromSeg;
            console.log(`[DASH→HLS] Fixed-duration segments (Live): window=${count} segs × ${(stDuration/timescale).toFixed(3)}s starting seg#${fromSeg}`);
          }
        }
      }
      ...
      const effectiveStartNum = finalSegments._liveStartNum != null ? finalSegments._liveStartNum : startNum;
      sets.push({ contentType, reprId, bandwidth, width, height, codecs,
                  timescale, initTmpl, mediaTmpl, startNum: effectiveStartNum, segments: finalSegments });
    }
  }
  return sets;
}
...
function buildMediaPlaylist(set, mpdBase, sessionId, isLive = false, track = 'unknown') {
  const seg = (cdnRelative) => {
    ...
    return `${globalThis.APP_URL}/proxy/segment?url=${encodeURIComponent(abs)}&s=${sessionId}`;
  };

  // Supports $Number$, $Number%0Nd$, $Time$, and $RepresentationID$
  const expand = (tmpl, n, t) => {
    ...
    if (t !== undefined && t !== null) {
      s = s.replace(/\$Time\$/g, String(t));
    }
    return s;
  };

  const allSegs = set.segments; // Array<{d, t}> — duration and timestamp per segment

  // Helper: get duration value from a segment entry (handles both {d,t} and legacy number)
  const segD = (seg) => (typeof seg === 'object' && seg !== null) ? seg.d : seg;
  const segT = (seg) => (typeof seg === 'object' && seg !== null) ? seg.t : null;
  ...
  const maxDur    = Math.max(...useSegs.map(segD)) / set.timescale;
  const targetDur = Math.ceil(maxDur);
  const initUrl   = seg(expand(set.initTmpl, 0, segT(useSegs[0])));
  ...
  for (const s of useSegs) {
    ...
    pl += seg(expand(set.mediaTmpl, segNum, t)) + '\n';
    segNum++;
  }
  if (!isLive) {
    pl += '#EXT-X-ENDLIST\n';
...
router.get("/dash", async (req, res) => {
  ...
  try {
    ...
    const forceLive = querySports === '1' || url.toLowerCase().includes('/live/') || url.toLowerCase().includes('nitro') || url.toLowerCase().includes('aiv-cdn');
    ...
    if (cachedEntry) {
      ...
      // Validate that we actually got a DASH MPD, not an error page
      const mpdTrimmed = mpdRaw.trimStart();
      const looksLikeMpd = mpdTrimmed.startsWith('<?xml') || mpdTrimmed.startsWith('<MPD') || mpdTrimmed.includes('<MPD ');
      if (!looksLikeMpd) {
        const statusCode = resp.status || 200;
        console.error(`[DASH→HLS] Received non-MPD response (status=${statusCode}, length=${mpdRaw.length}). URL likely expired.`);
        return res.status(502).send(`Stream URL expired or unavailable (CDN returned ${statusCode}). Please reload the sports page to get a fresh stream URL.`);
      }
      sets = parseDashAdaptationSets(mpdRaw);
      isLiveMpd = /type\s*=\s*"dynamic"/i.test(mpdRaw) || !/mediaPresentationDuration/i.test(mpdRaw) || forceLive;
    ...
    const proxyDash = (t) =>
      `${globalThis.APP_URL}/proxy/dash?url=${urlEnc}&ref=${refEnc}&cookie=${cookieEnc}&track=${t}${uaEnc}${origEnc}${keyEnc}${sportsEnc}`;
...
const _mpdHandler = async (req, res) => {
  ...
  try {
    ...
    if (cached && cached.expiresAt > Date.now()) {
      ...
      if (resp.status !== 200) {
        console.error(`[MPD-Proxy] Failed to fetch MPD manifest: status ${resp.status} for ${url}`);
        return res.status(resp.status).end(`Failed to fetch MPD manifest: status ${resp.status}`);
      }
    ...
    const sess2 = _sessions.get(session);
    if (sess2) {
      if (!sess2.decryptionKey && queryKey && queryKey.length === 32) {
        sess2.decryptionKey = queryKey;
        console.log(`[MPD-Proxy] Stored ClearKey for session ${session} (key=${queryKey.slice(0,8)}...)`);
      }
    }

    const proxyPfx = `${globalThis.APP_URL}/proxy/cdn/${session}/`;
...
router.get('/cdn/:session/:file', async (req, res) => {
  ...
  try {
    ...
    const cdnUrl  = cdnBase + file + queryStr;
    if (!cdnBase) {
...
router.get(["/video", "/video/:filename"], async (req, res) => {
  ...
  const isGoogleDriveCdn = url.includes('googleusercontent.com') || url.includes('google.com');
  if (isGoogleDriveCdn) {
    console.log(`[Proxy] Redirecting Google CDN link directly to bypass rate limits and enable seeking: ${url}`);
  ...
  const reqHeaders = {
    'User-Agent': req.query.ua || 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/137.0.0.0 Safari/537.36',
    'Accept': '*/*',
